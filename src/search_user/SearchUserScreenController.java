...
public class SearchUserScreenController {
	...
	public boolean Search_User(String nameToSearch) throws SQLException {
		String query = "SELECT * FROM UserData WHERE Username = '"+nameToSearch+"'";
	...
	public boolean Edit_User(String nameToSearch) {
		...
		try {
			if(rs.next()) {
				String query = "UPDATE UserData SET Username = '"+userName.getText()+"', "
						+ "Email = '" +email.getText()  + "', " 
	...
	public static boolean Delete_User(String nameToSearch) {
		...
		String query = "DELETE FROM UserData WHERE Username = '"+nameToSearch+"'";
